...
# cmc_bpx2fod - transforms bpx outputs to SH-based FOD
#
...
def parse_cmdline_args():
    ...
    # Optional arguments
    p.add_argument("--mask", required=False,
                   help="brain mask")
    p.add_argument("--order", required=False, default=8, type=int,
                   help="order of the spherical harmonics (default=8)")
...
def main():
    ...
    func = partial(process_voxel, ths=ths, phs=phs, fs=fs)
    results = []

    with parallel_backend("threading", n_jobs=n_workers):
        verbosity = 13 if args.verbose else 0
        results = Parallel(verbose=verbosity)(delayed(func)(vox) for vox in voxels)
